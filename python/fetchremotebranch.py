...
try:
	...
	pieces=full_branch.split("/")
	remote=pieces[0]
	branch=pieces[1]
	command="%s %s %s %s"%(sanitize_str(options.git),"fetch",remote,branch)
	...
	sys.stderr.write("The fetch remote branch command threw this error: " + str(e))
	sys.stderr.write("\ncommand: %s\n" % command)

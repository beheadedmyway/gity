...
try:
	from _argv import *
	if not options.misc: raise Exception("Gitty Error: The push tag to command requires a remote and a branch.")
	tag=sanitize_str(options.misc[0])
	remote=sanitize_str(options.misc[1])
	command="%s %s %s refs/tags/%s:refs/tags/%s"%(options.git,"push",remote,tag,tag)

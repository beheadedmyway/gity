...
try:
	from _argv import *
	command = "%s %s" % (options.git,"remote -v show")
	...
	if res[-1] == "": res.pop()
	for line in res:
		match = re.match("^([a-zA-Z0-9 ]{1,})\s([\.://@\\a-zA-Z0-9]{1,})\s",line)
		name = match.group(1)
		if names.get(name,False): continue
		names[name]=True #kill dups
		url=match.group(2)
		finals.append({"name":name,"url":url})
	sys.stdout.write(json.dumps(finals))
	exit(0)
except Exception, e:
	sys.stderr.write("The remotes command through this error: " + str(e))
	exit(84)
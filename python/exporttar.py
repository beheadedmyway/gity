...
try:
	...
	gitcommand="archive --format=tar --prefix=%s/ %s > %s"%(sanitize_str(options.misc[0]),sanitize_str(options.misc[0]),sanitize_str(options.misc[1]))
	command="%s %s"%(options.git,gitcommand)
	rcode,stout,sterr=run_command(command)
	rcode_for_git_exit(rcode,sterr)
	exit(0)
except Exception, e:
	sys.stderr.write("The export tar command through this error: " + str(e))
	sys.stderr.write("\ncommand: %s" % command)
